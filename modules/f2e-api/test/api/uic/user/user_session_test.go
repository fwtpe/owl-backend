...
func TestUserGetInfo(t *testing.T) {
	...
	Convey("get user info", t, func() {
		...
		Convey("only admin can get user info by id", func() {
			w, r = NewTestContextWithDefaultSession("GET", "/api/v1/user/u/3", nil)
			routes.ServeHTTP(w, r)
			respBody := w.Body.String()
			So(respBody, ShouldContainSubstring, "only admin user")
			So(w.Code, ShouldEqual, 400)
			CleanSession(r)
		})
		Convey("admin can get user info by id", func() {
