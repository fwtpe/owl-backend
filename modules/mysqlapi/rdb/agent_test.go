...
type TestNqmAgentHeartbeatTargetListSuite struct{}

var _ = Suite(&TestNqmAgentHeartbeatTargetListSuite{})

// Tests the getting of log of cached list of ping
func (suite *TestNqmAgentHeartbeatTargetListSuite) TestGetCacheLogOfPingList(c *C) {
	testCases := []*struct {
		agentId int32
		checker Checker
	}{
...
func (suite *TestNqmAgentHeartbeatTargetListSuite) TestBuildCacheOfPingList(c *C) {
	testCases := []*struct {
		agentId                      int32
		checkedTime                  string
...
func (suite *TestNqmAgentHeartbeatTargetListSuite) TestGetPingList(c *C) {
	testCases := []*struct {
		agentId        int32
	...
	for i, testCase := range testCases {
		...
		for i, target := range testedResult {
			...
			c.Assert(target.ID, Equals, testCase.expectedResult[i], comment)
		}
		// :~)
	}
}
...
func (suite *TestNqmAgentHeartbeatTargetListSuite) TestUpdateAccessTime(c *C) {
	accessTime := time.Now()
...
func (suite *TestNqmAgentHeartbeatTargetListSuite) TestTriggersOfFiltersForPingTask(c *C) {
	testedCases := []*struct {
		sqls                            []string
...
func (suite *TestNqmAgentHeartbeatTargetListSuite) Test_vw_enabled_targets_by_ping_task(c *C) {
	testCases := []*struct {
		pingTaskId           int
...
func (s *TestNqmAgentHeartbeatTargetListSuite) SetUpTest(c *C) {
	var executeInTx = DbFacade.SqlDbCtrl.ExecQueriesInTx

	switch c.TestName() {
	case "TestNqmAgentHeartbeatTargetListSuite.Test_vw_enabled_targets_by_ping_task":
		executeInTx(
			`
	...
	case "TestNqmAgentHeartbeatTargetListSuite.TestTriggersOfFiltersForPingTask":
		executeInTx(
			`
			INSERT INTO owl_name_tag(nt_id, nt_value)
	...
	case "TestNqmAgentHeartbeatTargetListSuite.TestGetCacheLogOfPingList":
		executeInTx(
			`
	...
	case "TestNqmAgentHeartbeatTargetListSuite.TestBuildCacheOfPingList":
		executeInTx(
			`
        ...
        (80930, 'pl-tg-5', '10.25.61.75', 7, FALSE, TRUE, TRUE) -- Same IP Address with agent[50761]
			`,
			`
			INSERT INTO nqm_ping_task(
				pt_id, pt_period
	...
	case "TestNqmAgentHeartbeatTargetListSuite.TestGetPingList":
		executeInTx(
			`
			INSERT INTO owl_group_tag(gt_id, gt_name)
	...
	case "TestNqmAgentHeartbeatTargetListSuite.TestUpdateAccessTime":
		executeInTx(
			`
			INSERT INTO host(id, hostname, agent_version, plugin_version)
...
func (s *TestNqmAgentHeartbeatTargetListSuite) TearDownTest(c *C) {
	var executeInTx = DbFacade.SqlDbCtrl.ExecQueriesInTx
	...
	case "TestNqmAgentHeartbeatTargetListSuite.TestBuildCacheOfPingList":
		executeInTx(
			...
			"DELETE FROM nqm_target WHERE tg_id >= 80921 AND tg_id <= 80930",
		)
	case "TestNqmAgentHeartbeatTargetListSuite.TestGetPingList":
		executeInTx(
			...
			"DELETE FROM nqm_target WHERE tg_id >= 99021 ANd tg_id <= 99023",
